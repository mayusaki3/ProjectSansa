[目次](../../../目次.md) > テスト仕様 > テストマトリクス> Webサービス > 認証・セッション (M01) テストマトリクス

# Webサービス / 認証・セッション (M01) テストマトリクス

本書は、認証・セッションモジュール (M01) に関するテスト観点と  
...
本マトリクスのテストケースは、共通テスト基盤 [sansa-testkit](../../../テスト基盤/sansa-testkit/目次.md)
...
- 形式: `Mxx:Ty-zz` など、既存の sansa-auth で使用している形式に準拠
  - 例: `M01:T01-01`（M01 モジュール / テストグループ01 / ケース01）
- UT/IT の区別は、テストクラス名・パッケージ、もしくはプレフィックスで区別
  - 例：`UT-01-001`, `IT-01-001` など  
    （実際の運用は既存ルールに合わせて記載・調整）
